...
## 자동 업데이트 모드
- 목표: **네가 버튼 누를 일 없이** 자동 갱신
- 동작 방식: push 이벤트는 모든 브랜치에서 감지하지만, **실제 Pages 배포는 기본 브랜치(main)에서만 실행**
- 장점: 작업 브랜치에서 불필요한 실패 배포(빨간 X)가 줄어듦
- 결과: 코덱스가 main에 반영하면 사이트는 자동 갱신됨 (수동 배포 버튼 불필요)
